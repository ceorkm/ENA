...
function readOrbMode() {
  try { return localStorage.getItem("ena.orbMode") !== "0"; } catch (e) { return true; }
}
...
function CollapsedOrb({ onExpand, busy }) {
  ...
  function onDown(e) {
    ...
    function up() { cleanup(); if (!dragged) onExpand(); }
    function cleanup() {
  ...
  return (
    <button className={"ena-collapsed" + (busy ? " is-busy" : "")} onMouseDown={onDown} title={busy ? "Working… click to open" : "Drag to move · click to open"}>
      <img src="ena-orb.png" alt="ENA" draggable={false} />
      {busy && <span className="ena-collapsed__ring" />}
    </button>
  );
}
...
function AppRoot() {
  const [onboarded, setOnboarded] = useStateApp(() => {
    try { return localStorage.getItem("ena.onboarded") === "1"; } catch (e) { return false; }
  });
  ...
  useEffectApp(() => {
    function onBusy(e) { setBusy(!!(e && e.detail)); }
  ...
  function finishOnboarding() {
    try { localStorage.setItem("ena.onboarded", "1"); } catch (e) {}
    setOnboarded(true);
  }
  ...
  useEffectApp(() => {
    ...
    t.event.listen("ena-toggle", () => {
      ...
      if (orbModeRef.current) {
        if (aw) { Promise.resolve(aw.show()).catch(() => {}); if (aw.setFocus) Promise.resolve(aw.setFocus()).catch(() => {}); }
        setCollapsed((c) => !c);
        HONE.refloat && HONE.refloat();
      } else if (aw && aw.isVisible) {
        aw.isVisible().then((v) => { if (v) { aw.hide(); } else { aw.show(); aw.setFocus && aw.setFocus(); HONE.refloat && HONE.refloat(); } }).catch(() => {});
      }
    }).then((f) => { un = f; }).catch(() => {});
    return () => { if (un) un(); };
  }, []);
  ...
  function handleCollapse() {
    if (orbModeRef.current) setCollapsed(true);
    else { const aw = appWin(); if (aw) aw.hide(); }
  }
  ...
  let body;
  if (!onboarded) {
    body = <Onboarding variant="native" onDone={finishOnboarding} />;
  } else {
    const hidden = orbMode && collapsed;
    body = (
      <React.Fragment>
        <div className="ena-barwrap" style={{ display: hidden ? "none" : "block" }}>
          <FloatingBar variant="native" initialView="resting" onCollapse={handleCollapse} />
        </div>
        {hidden && <CollapsedOrb onExpand={() => setCollapsed(false)} busy={busy} />}
      </React.Fragment>
    );
  }

  return <div className="ena-app">{body}</div>;
}
